...
@WebServlet(value = "/mySold.do")
public class SoldBoardList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<SailBoardVO> soldboardList = boardService.getTotalSoldList(mem_no);
		List<AtchSmpleVO> smpleList = atchFileService.getAtchSmple();

		req.setAttribute("soldboardList", soldboardList);
		req.setAttribute("smpleList", smpleList);

		req.getRequestDispatcher("/mySold.jsp").forward(req, resp);
	}
