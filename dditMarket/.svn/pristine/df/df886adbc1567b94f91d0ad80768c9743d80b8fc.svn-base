...
List<SailBoardVO> buyboardListNl = (List<SailBoardVO>) request.getAttribute("buyboardListNl");
List<SailBoardVO> buyboardList = (List<SailBoardVO>) request.getAttribute("buyboardList");
...
<body>
	...
	<span class="cute-font-regular">
		<div class="text-center text-black">
			<h1 class="display-6 fw-bolder" style="font-size: 50px">나의 구매 상품</h1>
		</div> <%
 	int sailSizeNl = buyboardListNl.size();
 int sailSize = buyboardList.size();
 if (sailSizeNl > 0 || sailSize > 0) {
 %>
		<section class="py-5">
			<div class="container px-4 px-lg-5 mt-5">
				<div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
					<%
						for (SailBoardVO sbd : buyboardListNl) {
					%>
					<div class="col mb-5">
 ...
 %> <!-- Product image--> <!-- Product details-->
								...
								<div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
									<div class="text-center">
										<a class="btn btn-outline-dark mt-auto" href="<%=request.getContextPath()%>/reviewinsert.do?sale_no=<%=sbd.getSale_no()%>" onclick="window.open(this.href, '_blank', 'width=800, height=600'); return false;">후기 작성</a>
									</div>
								</div>
							</a>
						</div>
					...
					%>
					<%
						for (SailBoardVO sbd : buyboardList) {
					%>
					<div class="col mb-5">
						<div class="card h-100">
							<a class="navbar-brand" href="<%=request.getContextPath()%>/saleDetail.do?sale_no=<%=sbd.getSale_no()%>&mem_no=<%=loginCd.getMem_no()%>"> <%
 ...
 %> <!-- Product image--> <!-- Product details-->
								...
								<div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
									<div class="text-center">
										<a class="btn btn-outline-dark mt-auto" href="<%=request.getContextPath()%>/deletereview.do?sale_no=<%=sbd.getSale_no()%>" style="background-color: pink">후기 삭제</a>
									</div>
								</div>
							</a>
						</div>
